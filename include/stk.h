...
class Kernel : public IKernel, public IKernelService, private IPlatform::EventHandler
{
	class KernelTask : public IKernelTask, public util::DListEntry<KernelTask>
	{
		friend class Kernel;

	public:
		typedef util::DListEntry<KernelTask> ListEntryType;
		...
		IKernelTask *GetNext()
		{
			// TO-DO: remove condition by linking tail to the first entry
			KernelTask *next = (KernelTask *)ListEntryType::GetNext();
			return (next != NULL ? next : (KernelTask *)this->GetHead()->GetFirst());
		}
...
public:
	...
	void Start(uint32_t resolution_us)
	{
		...
		m_task_now = (KernelTask *)m_tasks.GetFirst();
...
protected:
	...
	typedef util::DListHead<KernelTask> TaskListT;

	IPlatform           *m_platform;        //!< platform driver
	ITaskSwitchStrategy *m_switch_strategy; //!< task switch strategy
	TaskStorageT         m_task_storage;    //!< task storage
	TaskListT            m_tasks;           //!< available tasks
