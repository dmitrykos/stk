...
- **Nesting Support**: Both `Mutex` and `Spinlock` support recursive acquisition from the same thread.
...
### 2. Spinlock (`stk::sync::Spinlock`)
A high-performance recursive spinlock for very short critical sections.
- **Configurable Spin Count**: Busy-waits for N iterations before calling `stk::Yield()`, preventing total system stall while waiting for the lock.
...
### 3. Condition Variable (`stk::sync::ConditionVariable`)
...
### 5. Semaphore (`stk::sync::Semaphore`)
...
### 6. Pipe (`stk::sync::Pipe<T, Capacity>`)
...
* **`Event::Set()`, `Event::Pulse()`, `Event::Reset()`, `Event::TryWait()`**
* **`Semaphore::Signal()`**
* **`ConditionVariable::NotifyOne()`, `ConditionVariable::NotifyAll()`, `ConditionVariable::Wait(NO_WAIT)`**
